...
$dir_mod_site = $_SERVER['DOCUMENT_ROOT'] . '/9.site/' . $vv['folder'] . '/module/' . $vv['now_level']['cfg.level'] . '/tpl_smarty/';
...
// вход по соц сервису
        elseif (isset($_POST['token']{10})) {
            ...
            $access_user = Nyos\mod\lk::enterSoc($db, $vv['folder'], $_POST['token']);
//f\pa($access_user); 
//die();
...
//$status = '';
            ...
            if (isset($access_user['user']['uid']))
                $sql1 = $db->sql_query('SELECT * FROM gm_user WHERE `folder` = \'' . $vv['folder'] . '\' AND `soc_web_id` = \'' . addslashes($access_user['user']['uid']) . '\' LIMIT 1 ; ');

//echo $status;
//echo $db->sql_numrows($sql1);
//die();
// если есть запись в бд о пользователе
            if ($db->sql_numrows($sql1) == 1) {

                $_SESSION['now_user'] = $db->sql_fr($sql1);

//f\pa($_SESSION['now_user']); 
//die();

                $sql = $db->sql_query('SELECT * FROM `gm_user_option` WHERE `user` = ' . $_SESSION['now_user']['id'] . ' ; ');
...
// регаем или входим
//                else {
//                    require_once dirname(__FILE__) . '/v.3.smarty.user_enter_reg.php';
//                }
            }
...
//die('<html><head><meta http-equiv="refresh" content="3;http://' . $_SERVER['HTTP_HOST'] . $vv['now_level']['goto_after_enter'].'"></head>'.
                die('<html><head><meta http-equiv="refresh" content="0;https://' . $_SERVER['HTTP_HOST'] . $vv['now_level']['goto_after_enter'] . '"></head>' .
                        '<body><br/><br/><br/><center>секунду пожалуйста</body></html>');

                header('Location: https://' . $_SERVER['HTTP_HOST'] . $vv['now_level']['goto_after_enter']);
                exit;
            }
            //
